...
Porém com o lançamento do SDK 4.0 no final do mês de Agosto melhorias foram implementadas no \emph{framework MapKit}, sendo agora possível a implementação de uma camada gráfica encima do mapa. A partir do estudo dessa camada, conseguimos implementar uma solução mais dinâmica onde já não era mais necessário o calcular novamente todas marcações para cada mudança no mapa.
