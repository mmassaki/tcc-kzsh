...
O arquivo de imagem do evento é obtido via POST \emph{multipart/form-data} e os dados binário da imagem são armazenados diretamente no banco de dados.

O cliente pode receber os dados resultantes da criação do novo evento através de uma página html ou em dados estruturados em XML ou JSON.
...
Esta requisição é tratada pelo método \emph{index}, descrito a seguir, da classe \emph{EventosController}.
\\
\begin{lstlisting}[language=Ruby]
  def index
...
Da mesma forma que a criação de novo evento os eventos podem ser obtidos como uma página html ou em estruturas JSON ou XML.
...
O envio do registro é feito através de um POST para o endereço \url{betterway.co.cc/registros} e o método \emph{create}, descrito a seguir, da classe \emph{RegistrosController} é o responsável por manipular essas requisições.
\\
\begin{lstlisting}[language=Ruby]
def create
...
Os dados do resultado da criação do novo registro podem ser obtidos através de uma página html ou em dados estruturados em JSON ou XML.
...
A classe \emph{RotaController}, responsável pela criação de rota no servidor é descrita a seguir.
\\
\begin{lstlisting}[language=Ruby]
...
\begin{figure}
	\centering
	\includegraphics[scale=0.7]{img/iphone/BetterWayDiagramaClasseRota.pdf}
